import { SAVE_REAL_ESTATE_FORM, UPDATE_COMPAIGN } from "./allActions.js";
import axios from "axios";
import { toastr } from "react-redux-toastr";
export const saveRealEstateForm = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}saveRealEstateForm`,
      data: data,
    };
    axios(options)
      .then((response) => {
...
export const updateCompaign = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}updateRealEstatetemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
