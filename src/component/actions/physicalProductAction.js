import {
  SAVE_TEMPLATE_PHYSICAL_PRODUCT,
  UPDATE_COMPAIGN,
} from "./allActions.js";
import axios from "axios";
import { toastr } from "react-redux-toastr";
export const savePhysicalProductForm = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}physicalProductTemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
        if (response.data.success === true) {
          dispatch({
            type: SAVE_TEMPLATE_PHYSICAL_PRODUCT,
...
export const updateCompaign = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}updatePhysicalProductTemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
