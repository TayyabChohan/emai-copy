import { SAVE_BLOG_SEQUENCE_FORM, UPDATE_COMPAIGN } from "./allActions";
import axios from "axios";
import { toastr } from "react-redux-toastr";

export const saveBlogSequenceForm = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}saveBlogSequenceForm`,
      data: data,
    };
    axios(options)
      .then((response) => {
...
export const updateCompaign = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}updateBlogSequencetemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
