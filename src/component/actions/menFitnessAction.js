import { SAVE_MENFITNESS_FORM, UPDATE_COMPAIGN } from "./allActions.js";
import axios from "axios";
import { toastr } from "react-redux-toastr";
export const saveMenFitnessForm = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}saveMenFitnessForm`,
      data: data,
    };
    axios(options)
      .then((response) => {
...
export const updateCompaign = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}updateMenfitnessTemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
