import { SAVE_FINANCE_FORM, UPDATE_COMPAIGN } from "./allActions.js";
import axios from "axios";
import { toastr } from "react-redux-toastr";
export const saveFinanceForm = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}saveFinanceForm`,
      data: data,
    };
    axios(options)
      .then((response) => {
...
export const updateCompaign = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}updateFinanceTemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
