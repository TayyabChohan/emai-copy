...
export const saveSalesLetterPromoSequenceForm = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}SalesLetterPromoTemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
        if (response.data.success === true) {
          dispatch({
            type: SAVE_TEMPLATE_SALES_LETTER_PROMO,
...
export const updateCompaign = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}updateSalesLetterPromoTemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
