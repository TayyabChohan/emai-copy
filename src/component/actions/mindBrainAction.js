import { SAVE_MIND_BRAIN, UPDATE_COMPAIGN } from "./allActions.js";
import axios from "axios";
import { toastr } from "react-redux-toastr";
export const saveMindBrainForm = (data) => {
  return (dispatch) => {
    const option = {
      url: `${process.env.REACT_APP_API_URL}mindBrainTemplate`,
      data: data,
      method: "POST",
    };
    axios(option)
...
export const updateCompaign = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}updateMindBrainTemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
