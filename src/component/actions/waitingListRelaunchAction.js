...
export const saveWaitingListRelaunchForm = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}waitingListRelaunchTemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
        if (response.data.success === true) {
          dispatch({
            type: SAVE_TEMPLATE_WAITING_LIST_RELAUNCH,
...
export const updateCompaign = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}updateWaitingListRelaunchTemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
