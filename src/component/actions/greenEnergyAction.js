import { SAVE_GREEN_ENERGY, UPDATE_COMPAIGN } from "./allActions.js";
import axios from "axios";
import { toastr } from "react-redux-toastr";

export const saveGreenEnergyForm = (data) => {
  return (dispatch) => {
    const option = {
      url: `${process.env.REACT_APP_API_URL}greenEnergyTemplate`,
      data: data,
      method: "POST",
    };
    axios(option)
...
export const updateCompaign = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}updateGreenEnergyTemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
