import { SAVE_DIRECT_REVENUE, UPDATE_COMPAIGN } from "./allActions.js";
import axios from "axios";
import { toastr } from "react-redux-toastr";

export const saveDirectRevenueForm = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}saveDirectRevenueForm`,
      data: data,
    };
    axios(options)
      .then((response) => {
...
export const updateCompaign = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}updateDirectRevenuetemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
