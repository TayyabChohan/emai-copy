...
export const saveWebinarReplaySeqForm = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}webinarReplaySeqTemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
        if (response.data.success === true) {
          dispatch({
            type: SAVE_TEMPLATE_WEBINAR_REPLAY_SEQ,
...
export const updateCompaign = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}updateWebinarReplaySeqTemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
