...
export const saveShoppingCartAbandonmentForm = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}shoppingCartAbandonmentTemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
        if (response.data.success === true) {
          dispatch({
            type: SAVE_TEMPLATE_SHOPPING_CART_ABANDONMENT,
...
export const updateCompaign = (data) => {
  return (dispatch) => {
    const options = {
      method: "POST",
      url: `${process.env.REACT_APP_API_URL}updateShoppingCartAbandonmentTemplate`,
      data: data,
    };
    axios(options)
      .then((response) => {
