...
exports.leadMagnetSequenceReportTemplate = (req, res) => {
  Campaign.create({
    description: "",
    downloads: 0,
    ...
    .then((campainData) => {
      LmsReport.create({
        ...
        lms_page_count: req.body.lms_page_count,
          ...
          .then((emailData) => {
            for (let i = 0; i < emailData.length; i++) {
              ...
              let mapObj = {
                ...
                "#LmsPageCount": req.body.lms_page_count,
              ...
              customEmailBody = customEmailBody.replace(
                /#LmsTitle|#LmsTopic|#LmsPageCount|#LmsSupportEmail|#YourName|#Benefits/gi,
                function (matched) {
                  return mapObj[matched];
                }
              );
              customEmailSubject = customEmailSubject.replace(
                /#LmsTitle|#LmsTopic|#LmsURL|#LmsPageCount|#LmsSupportEmail|#YourName|#Benefits/gi,
                function (matched) {
                  return mapObj[matched];
                }
              );
...
exports.updateLeadMagnetSequenceReportTemplate = (req, res) => {
  LmsReport.update(
    {
      name: req.body.name,
      lms_topic: req.body.lms_topic,
  ...
  EmailCustom.findAll({
    ...
    .then((emailData) => {
      ...
      for (let i = 0; i < emailData.length; i++) {
        ...
        mystring = mystring.replace(
          /<span var="#VarLmsPageCount">(.*?)<\/span>/g,
          '<span var="#VarLmsPageCount">' + req.body.lms_page_count + "</span>"
        );
        mystring = mystring.replace(
